...
export function BlockLimbo({position = [0, 0, 0]})
{
  ...
  useFrame((state) => {
    ...
    //NOTE: setNextKinematicTranslationの引数はベクトルで指定する
...
export function BlockEnd({position = [0, 0, 0]})
{
  ...
  return <group position={ position }>
    <mesh
      geometry={ boxGeometry }
      scale={ [4, 0.2, 4] }
      //MEMO: あえてyは0にしている。最後にジャンプしてゴールさせるようにするため
      position={[0, 0, 0]}
      receiveShadow
      material={ floor1Material }
    />
    <RigidBody type='fixed' colliders='hull' position={ [0, 0.25, 0] } restitution={ 0.2 } friction={ 0.2 }  >
